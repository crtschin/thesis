...
Proof.
  ...
  (* - apply IHt1 in H2.
    apply IHt2 in H4.
    destruct H2; destruct H4. subst.
    inversion H; subst.
    + right. exists ([x0:=t2]t). eapply ST_AppAbs. assumption.
    + left. apply progress in Ht. destruct Ht. assumption. *)
...
Proof.
  ...
  pose proof (free_in_context _ _ _ _ contra H).
...
Proof.
  ...
  pose proof progress as Hprog.
  pose proof preservation as Hpres.
  induction Hmulti.
  - apply Hprog in Hhas_type.
    destruct Hhas_type.
    + contradiction Hnot_val.
    + contradiction Hnf.
  - apply IHHmulti.
      eapply Hpres; try apply Hhas_type; assumption.
      assumption.
      assumption.
Qed.
...
Theorem type_arrow_eq : forall T1 T1' T2 T2',
  T1 = T2 ->
  T1' = T2'  ->
...
Proof with eauto.
  ...
  - assert (T0 = T11)...
    subst. apply (IHe1 _ _ _ H4) in H10.
    inversion H10...
  - pose proof type_arrow_eq as Heq.
    apply Heq...
Qed.
