...
Corollary preservation_multi : forall t t' T,
     has_type empty t T  ->
     t -->* t'  ->
     has_type empty t' T.
Proof.
  ...
  - assumption.
  - apply IHmulti.
    eapply preservation. apply H1. assumption.
Qed.
 ...
 induction T;  intros t t' E Rt; unfold R; fold R; unfold R in Rt; fold R in Rt;
  ...
  - split; try split.
    ...
    + eapply step_preserves_halting in E.
      apply E. assumption.
    + destruct RRt as [r [s [Hp [Hvr [Hvs [HT1 HT2]]]]]].
      exists r.
      exists s. split...
      inversion Hp; subst.
      * assert (value (pair r s)).
        { constructor... }
        apply value__normal in H.
        unfold step_normal_form in H. contradiction H...
      * rewrite (step_deterministic _ _ _ E H)...
Qed.
...
Proof.
  induction T; intros t t' E Rt.
  - simpl. intros [He [Hh Ht]]. clear Ht.
    split.
    + eauto.
    + split; auto.
      eapply step_preserves_halting. apply Rt. assumption.
  - simpl. intros [He [Hh Ht]].
    split; try split.
    + assumption.
    + eapply step_preserves_halting. apply Rt. assumption.
    + intros. eapply IHT2. simpl.
      * eapply T_App. apply E. apply R_typable_empty. assumption.
      * constructor. apply Rt.
      * apply Ht. assumption.
  - simpl. intros [Hp [Ht Ha]].
    split; try split.
    + assumption.
    + apply step_preserves_halting in Rt.
      apply Rt. assumption.
    + destruct Ha as [r [s Ha]].
      exists r. exists s.
      intros. destruct Ha as [Hpr [Hr [Hs [HT1 Ht2]]]].
      split; auto.
      pose proof (multi_step step _ _ _ Rt Hpr).
      assumption.
Qed.
...
Proof with eauto.
  induction t; simpl.
  - intros. destruct (eqb_stringP x s). subst.
    contradiction H. constructor. reflexivity.
  - intros. assert (H': ~ appears_free_in x t1 /\ ~ appears_free_in x t2).
    { split; unfold not; intros; apply H; constructor; assumption. }
    destruct H'.
    eapply IHt1 in H0. eapply IHt2 in H1.
    rewrite H0. rewrite H1. reflexivity.
  - intros x. destruct (eqb_stringP x s). reflexivity.
    intros.
    assert (H': ~ appears_free_in x t0).
    { unfold not. unfold not in H. intros. apply H.
      constructor...
    }
    intros. eapply IHt in H'. rewrite H'. reflexivity.
  - intros.
    assert (H': ~ appears_free_in x t1 /\ ~ appears_free_in x t2).
    { unfold not. unfold not in H. split; intros; apply H.
      - apply afi_pair1. assumption.
      - apply afi_pair2. assumption. }
    destruct H'.
    eapply IHt1 in H0. eapply IHt2 in H1.
    rewrite H0. rewrite H1.
    reflexivity.
  - intros.
    assert (H': ~ appears_free_in x t).
    { unfold not. unfold not in H. intros. apply H. constructor. assumption. }
    eapply IHt in H'. rewrite H'. reflexivity.
  - intros.
    assert (H': ~ appears_free_in x t).
    { unfold not. unfold not in H. intros. apply H. constructor. assumption. }
    eapply IHt in H'. rewrite H'. reflexivity.
  ...
  - reflexivity.
  - intros.
    assert (H': ~ appears_free_in x t1 /\ ~ appears_free_in x t2 /\ ~ appears_free_in x t3).
    { unfold not. unfold not in H. split; try split; intros; apply H.
      - apply afi_test0. assumption.
      - apply afi_test1. assumption.
      - apply afi_test2. assumption. }
    destruct H'. destruct H1.
    eapply IHt1 in H0. eapply IHt2 in H1. eapply IHt3 in H2.
...
Lemma msubst_fst : forall ss t,
  msubst ss (fst t) = fst (msubst ss t).
Proof.
 induction ss; intros.
   reflexivity.
   destruct a.
...
Lemma msubst_snd : forall ss t,
  msubst ss (snd t) = snd (msubst ss t).
Proof.
 induction ss; intros.
   reflexivity.
   destruct a.
...
Lemma msubst_test : forall ss t1 t2 t3,
  msubst ss (test t1 t2 t3) = test (msubst ss t1) (msubst ss t2) (msubst ss t3).
Proof.
 induction ss; intros.
   reflexivity.
   destruct a.
...
Lemma multistep_Pair1 : forall t t' t1,
  (t -->* t') -> (pair t t1) -->* (pair t' t1).
Proof.
  intros. induction H.
  - constructor.
...
Lemma multistep_Fst : forall v t t',
  value v -> (t -->* t') -> fst t -->* fst t'.
Proof.
  intros. induction H0.
  - constructor.
  - eapply multi_step; eauto.
...
Lemma multistep_Snd : forall v t t',
  value v -> (t -->* t') -> snd t -->* snd t'.
Proof.
  intros. induction H0.
  - constructor.
  - eapply multi_step; eauto.
...
Lemma multistep_Test1 : forall v t t' t1 t2,
  value v -> (t -->* t') -> test t t1 t2 -->* test t' t1 t2.
Proof.
  intros. induction H0.
  - constructor.
  - eapply multi_step; eauto.
...
Proof with eauto.
  ...
  - (* T_Prod *)
    ...
    unfold R. fold R. split.
    + constructor; apply R_typable_empty...
    + pose proof (R_halts P1) as [x1 [Hpr1 Hval1]].
      pose proof (R_halts P2) as [x2 [Hpr2 Hval2]].
      assert (pair (msubst env0 t1) (msubst env0 t2) -->* pair x1 x2).
      { eapply multi_trans.
        - eapply multistep_Pair1...
        - eapply multistep_Pair2...
      }
      unfold halts.
      split.
      * exists (pair x1 x2)...
      * exists x1. exists x2.
        pose proof multistep_preserves_R as Hpres.
        repeat (try split)...
  - rewrite msubst_fst.
    pose proof (IHHT c H env0 V) as P.
    unfold R in P. fold R in P.
    destruct P as [Hhas [Hhalts [r [s [Hp [Hr [Hs [Hrt1 Hrt2]]]]]]]].
    eapply multistep_preserves_R'.
    + eapply T_Fst...
    + eapply multistep_Fst...
    + eapply step_preserves_R'...
      * econstructor. eapply preservation_multi.
        apply Hhas... assumption.
  - rewrite msubst_snd.
    pose proof (IHHT c H env0 V) as P.
    unfold R in P. fold R in P.
    destruct P as [Hhas [Hhalts [r [s [Hp [Hr [Hs [Hrt1 Hrt2]]]]]]]].
    eapply multistep_preserves_R'.
    + eapply T_Snd...
    + eapply multistep_Snd...
    + eapply step_preserves_R'...
      * econstructor. eapply preservation_multi.
        apply Hhas... assumption.
  - rewrite msubst_tru.
    unfold R. repeat (try split)...
    exists tru...
  - rewrite msubst_fls.
    unfold R. repeat (try split)...
    exists fls...
  - pose proof (IHHT1 c H env0 V) as [HP1 [Hpr Hres1]].
    pose proof (IHHT2 c H env0 V) as P2.
    pose proof (IHHT3 c H env0 V) as P3.
    rewrite msubst_test.
    destruct Hpr as [t' [HPh1 HPv1]].
    eapply multistep_preserves_R'.
    + constructor...
      apply R_typable_empty...
      apply R_typable_empty...
    + eapply multistep_Test1. apply HPv1. apply HPh1.
    + induction HPv1. try solve_by_invert.
      * admit.
      * admit.
      * eapply step_preserves_R'.
        constructor...
        apply R_typable_empty...
        apply R_typable_empty...
        apply ST_TestTrue. apply P2.
      * eapply step_preserves_R'.
        constructor...
        apply R_typable_empty...
        apply R_typable_empty...
        apply ST_TestFalse. apply P3.
Admitted.
