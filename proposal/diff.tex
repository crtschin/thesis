...
%DIF DEL first_draft.tex   Mon Apr 20 01:15:17 2020
%DIF ADD proposal.tex      Mon Apr 20 01:19:56 2020
%DIF 2a2
...
\DIFdelbegin \DIFdel{We }\DIFdelend \DIFaddbegin \DIFadd{In this thesis, we will }\DIFaddend aim to formalize an extendable proof of an implementation of automatic differentiation on a \DIFdelbegin \DIFdel{simply typed }\DIFdelend \DIFaddbegin \DIFadd{simply-typed }\DIFaddend lambda calculus in the \<Coq> proof assistant, opening up further possibilities for formally proving the correctness of more complex language features in the future.
Our formalization is based on a recent proof by Stanton\DIFaddbegin \DIFadd{, }\DIFaddend Huot, and \Vakar{} \cite{huot2020correctness}.
...
\begin{itemize}
  \item \DIFdelbegin \DIFdel{Contribute a formalized }\DIFdelend \DIFaddbegin \DIFadd{Formalize the }\DIFaddend proof of forward-mode automatic differentiation \DIFaddbegin \DIFadd{specified by Stanton, Huot, and }\Vakar{} \DIFadd{\mbox{%DIFAUXCMD
...
\item%DIFAUXCMD
\DIFdelend Prove that well-known \DIFaddbegin \DIFadd{compile-time }\DIFaddend optimizations such as the partial evaluation, are correct with respect to automatic differentiation.
...
For the purpose of this thesis, however, this was deemed excessive and much too difficult and \DIFdelbegin \DIFdel{time consuming }\DIFdelend \DIFaddbegin \DIFadd{time-consuming }\DIFaddend to implement in a mathematically sound manner in \<Coq>.
\DIFdelbegin \DIFdel{As such, we chose to make use }\DIFdelend \DIFaddbegin \DIFadd{We avoided using many of the concepts dependent on traditional domain theory due to its limited support in \<Coq>.
...
, but these were incompatible with recent versions }\DIFaddend of \<Coq>\DIFdelbegin \DIFdel{'s existing types as denotations and base the relation on the denotations instead of the syntactic structures.
Due to the relative simplicity of the }\DIFdelend \DIFaddbegin \DIFadd{.
As a part of its type system, \<Coq> contains a set-theoretical model available under the sort \<Set>.
Because recursion, iteration and other concepts dealing with non-termination and partiality are left out of the scope of this thesis, this set-theoretical model was satisfactory for encoding the denotational semantics.
}

\DIFadd{Because we use the real numbers as the ground type in our }\DIFaddend language, we \DIFdelbegin \DIFdel{did not yet require domain theoretical concepts. If recursion or iteration were to be added to the language, it is currently expected that these would be needed .
}\DIFdelend \DIFaddbegin \DIFadd{also needed an encoding of the real numbers in Coq. The library for real numbers in \<Coq> has improved in recent times from one based on a completely axiomatic definition to one involving Cauchy sequences}\fancyfootnote{https://coq.inria.fr/library/Coq.Reals.ConstructiveCauchyReals.html}\DIFadd{. For the purposes of this thesis, however, we needed differentiability as the denotational result of applying the macro operation. Instead of encoding this by hand, we opted for the more comprehensive library \<Coquelicot>\mbox{%DIFAUXCMD
...
We currently mimic the base types used in the paper~\cite{huot2020correctness} extended with sum types, shown in \DIFdelbegin \DIFdel{snippet}\DIFdelend \DIFaddbegin \DIFadd{Snippet}\DIFaddend ~\ref{lst:stlc_types}.
...
\cite{barthe2020versatility} }\hspace{0pt}%DIFAUXCMD
in their }\DIFaddend proof. In a later section Stanton, Huot and \Vakar{} extended their language with sum and inductive types.
...
We will be looking to extend the current prototype proof with the continuation-based AD macro mentioned \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{by Stanton, Huot, and }\Vakar{}\DIFaddend \cite{huot2020correctness}.
