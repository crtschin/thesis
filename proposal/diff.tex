...
%DIF ADD proposal.tex      Fri Apr 24 15:36:15 2020
...
One of the principal techniques used in machine learning is back propagation, which calculates the gradient of a function. The \DIFdelbegin \DIFdel{idea being to use the gradient }\DIFdelend \DIFaddbegin \DIFadd{gradient itself is used }\DIFaddend in the gradient descent algorithm \DIFaddbegin \DIFadd{to optimize an objective function by determining the direction of steepest descent}\DIFaddend \cite{Baydin2015AutomaticDI}.
...
In the \DIFaddbegin \DIFadd{past }\DIFaddend 30 years since it has been released, research has contributed to extending the proof assistant with additional features such as inductive and co-inductive data types\cite{Coquand1990}, dependent pattern matching\cite{Sozeau2010} and advanced modular constructions for organizing \DIFdelbegin \DIFdel{colossal }\DIFdelend \DIFaddbegin \DIFadd{large }\DIFaddend mathematical proofs\cite{Sozeau2008}\cite{Mahboubi2013}.
...
In \<Coq>, one can normally write function definitions using either case-analysis as is done in other functional languages, or using \<Coq>'s tactics \DIFdelbegin \DIFdel{.
If }\DIFdelend \DIFaddbegin \DIFadd{language.
Using the standard case-analysis functionality can cause the code to be complicated and verbose, even more so when }\DIFaddend proof terms are present in the function \DIFdelbegin \DIFdel{definition, however, it is customary to write it using tactics because of the otherwise cumbersome and verbose code needed to pattern-match on the arguments. But this can be troublesome in the cases where the function signature is ambiguous, as it can be hard to recognize what the function then actually computes}\DIFdelend \DIFaddbegin \DIFadd{signature.
This has been caused by the previously poor support in Coq for dependent pattern matching.
Using the return keyword, one is able to vary the result type of a match expression. But due to requirement Coq used to have that case expressions be syntactically total, this could be very annoying to work with}\DIFaddend .
...
As an example, we will work through defining a length indexed list and a corresponding head function \DIFdelbegin \DIFdel{, which is well known to be partial}\DIFdelend \DIFaddbegin \DIFadd{limited to lists of length at least one in Snippet~\ref{lst:dt_ilist}}\DIFaddend .
Using the \<Coq> keyword return, it is possible to let the return type of a match \DIFdelbegin \DIFdel{expression }\DIFdelend \DIFaddbegin \DIFadd{expressions }\DIFaddend depend on the result of one of the type arguments.
This makes it possible to \DIFdelbegin \DIFdel{specify what the return type of the empty list should be.
In snippet~\ref{lst:dt_ilist}, we use the unit type which contains just oneinhabitant, \<tt>}\DIFdelend \DIFaddbegin \DIFadd{define an auxiliary function which, while total on the length of the list, has an incorrect return type. It namely returns the type unit if the input list had the length zero.
We can then use this auxiliary function in the actual head function by specifying that the list has length at least one.
It should be noted that more recent versions of Coq do not require that case expressions be syntactically total, so specifying that the input list has a length of at least zero is enough to eliminate the requirement for the zero-case}\DIFaddend .
...
Due to the intrinsic typeful representation this paper uses, much of this was invaluable when defining the substitution operators as the regular type checker in Coq often had difficulty \DIFdelbegin \DIFdel{when recognizing type equalities }\DIFdelend \DIFaddbegin \DIFadd{unifying dependently typed terms }\DIFaddend in certain cases.
...
The proof is done using a logical relation on the denotational semantics using \DIFaddbegin \DIFadd{the set-theoretic types in }\DIFaddend \<Coq> \DIFdelbegin \DIFdel{'s types }\DIFdelend as the underlying domain.
...
One possibility is to go towards a SSA (static single assignment) representation which has many benefits as a well-known possible internal representation for use in compilers\mbox{%DIFAUXCMD
...
We will be trying to extend the proof with the results achieved by Shaikhha, et. al.\mbox{%DIFAUXCMD
\cite{Shaikha2019}}\hspace{0pt}%DIFAUXCMD
