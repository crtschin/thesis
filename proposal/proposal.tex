...
\begin{itemize}
  ...
  \item Prove that well-known compile-time optimizations such as the partial evaluation, are correct with respect to automatic differentiation.
...
We avoided using many of the concepts dependent on traditional domain theory due to its limited support in Coq.
There are possible libraries we could have used which have resulted from experiments trying to encode domain theoretical models in Coq\cite{Benton2009}\cite{Dockins2014}, but these were incompatible with recent versions.
...
An Examples include the law of the excluded middle, $\forall A, A \vee \neg A$.
...
We will be looking to extend the current prototype proof with the continuation-based AD macro mentioned by Stanton, Huot, and \Vakar{}\cite{huot2020correctness}.
