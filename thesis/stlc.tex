\subsection{Simply Typed Lambda Calculus}\label{sec:formal_stlc}
  ...
  As expected, we include variables, applications and abstractions in the language using the \<var>, \<app> and \<abs> constructors.
  ...
  For real numbers, \<rval> is used to introduce real numbered constants and \<add> and \<mul> will be used to encode addition and multiplication.
  ...
  In other words, for any well-typed term $\Gamma \vdash t : \tau$, applying the forward-mode macro results in a well-typed term in the macro context, $\D(\Gamma) \vdash \D(t) : \D(\tau)$.
