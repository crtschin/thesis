...
Next, we will prove that the various program transformation rules they use in their system are sound.
...
As a small deviation from the rules given by Shaikhha, et al.\cite{Shaikha2019}, we explicitly include a set of simplification rules in the style of a natural semantics.
This is to facilitate partial evaluation on non-functional terms.
Partial evaluation on function terms is included as a separate transformation rule.
\cref{fig:rewrite_rules,fig:natural_infer} show, respectively, the rewrite rules we included in our language and the inference rules we used for our simplification rules.
